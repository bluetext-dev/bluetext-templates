...
Secret values land at the cluster Vault at `<path-prefix>/variants/{_default, <run-spec-variant>}/<value-id>` — one raw file per value, outside the system tree, never in git. Service variants reference them via `secrets.<name>.keys.<file>: secrets::<value-id>`; the deploy pipeline reads each file and projects it into a K8s `Secret`.
