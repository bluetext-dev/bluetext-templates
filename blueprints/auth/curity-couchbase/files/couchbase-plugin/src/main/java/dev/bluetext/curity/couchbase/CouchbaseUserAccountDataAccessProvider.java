...
public class CouchbaseUserAccountDataAccessProvider implements UserAccountDataAccessProvider {
    ...
    public static final String LINKED_ACCOUNT_COLLECTION_NAME = "curity-linked-accounts";
    private final CouchbaseExecutor _couchbaseExecutor;
    private final com.couchbase.client.java.Collection _linkedAccountCollection;

    public CouchbaseUserAccountDataAccessProvider(CouchbaseExecutor couchbaseExecutor) {
        _couchbaseExecutor = couchbaseExecutor;
        _linkedAccountCollection = couchbaseExecutor.getScope().collection(LINKED_ACCOUNT_COLLECTION_NAME);
    }
    ...
    public void link(String linkingAccountManager, String localAccountId,
        ...
        _logger.debug("Linking account: manager={}, local={}, domain={}, foreign={}",
                linkingAccountManager, localAccountId, foreignDomainName, foreignUserName);
        var record = new LinkedAccountRecord(linkingAccountManager, localAccountId,
                foreignDomainName, foreignUserName);
        String key = LinkedAccountRecord.key(linkingAccountManager, localAccountId,
                foreignDomainName, foreignUserName);
        _linkedAccountCollection.upsert(key, record);
    }

    @Override
    public Collection<LinkedAccount> listLinks(String linkingAccountManager, String localAccountId) {
        _logger.debug("Listing links: manager={}, localAccountId={}", linkingAccountManager, localAccountId);
        var config = _couchbaseExecutor.getConfiguration();
        String query = String.format(
                "SELECT `%s`.* FROM `%s`.%s.`%s` WHERE linkingAccountManager = $manager AND localAccountId = $localId",
                LINKED_ACCOUNT_COLLECTION_NAME, config.getBucket(), config.getScope(), LINKED_ACCOUNT_COLLECTION_NAME);
        var params = JsonObject.create()
                .put("manager", linkingAccountManager)
                .put("localId", localAccountId);
        ...
        for (var row : rows) {
            links.add(LinkedAccount.of(
                    (String) row.get("foreignDomainName"),
                    (String) row.get("foreignUserName")));
        }
        return links;
    }
    ...
    public @Nullable AccountAttributes resolveLink(String linkingAccountManager,
        ...
        _logger.debug("Resolving link: manager={}, domain={}, foreignId={}",
                linkingAccountManager, foreignDomainName, foreignAccountId);
        var config = _couchbaseExecutor.getConfiguration();
        String query = String.format(
                "SELECT `%s`.* FROM `%s`.%s.`%s` WHERE linkingAccountManager = $manager "
                        + "AND foreignDomainName = $domain AND foreignUserName = $foreignId",
                LINKED_ACCOUNT_COLLECTION_NAME, config.getBucket(), config.getScope(), LINKED_ACCOUNT_COLLECTION_NAME);
        var params = JsonObject.create()
                .put("manager", linkingAccountManager)
                .put("domain", foreignDomainName)
                .put("foreignId", foreignAccountId);
        var result = _couchbaseExecutor.executeParameterizedQuery(query, params);
        if (result.isEmpty()) {
            return null;
        }
        String localAccountId = (String) result.get(0).get("localAccountId");
        return _couchbaseExecutor.getByParameter(Parameters.USERNAME, localAccountId, null);
    }

    @Override
    public boolean deleteLink(String linkingAccountManager, String localAccountId,
                              String foreignDomainName, String foreignUserName) {
        _logger.debug("Deleting link: manager={}, local={}, domain={}, foreign={}",
                linkingAccountManager, localAccountId, foreignDomainName, foreignUserName);
        String key = LinkedAccountRecord.key(linkingAccountManager, localAccountId,
                foreignDomainName, foreignUserName);
        try {
            _linkedAccountCollection.remove(key);
            return true;
