...
package dev.bluetext.curity.couchbase.token;
...
import dev.bluetext.curity.couchbase.CouchbaseExecutor;
...
public final class CouchbaseTokenDataAccessProvider implements TokenDataAccessProvider {
    ...
    public long setStatusByTokenHash(String tokenHash, TokenStatus newStatus) {
        try {
            collection.mutateIn(tokenHash, List.of(MutateInSpec.replace("status", newStatus.name())));
            return 1;
