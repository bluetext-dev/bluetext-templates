...
import dev.bluetext.curity.couchbase.configuration.DBSetupRunners;
...
public class CouchbaseExecutor extends ManagedObject<CouchbaseDataAccessProviderConfiguration> {
    ...
    public CouchbaseDataAccessProviderConfiguration getConfiguration() {
        return configuration;
    }

    /**
     * Executes a parameterized N1QL query with named parameters for safe dynamic queries.
     */
    public List<Map<String, Object>> executeParameterizedQuery(String query, JsonObject parameters) {
        var options = QueryOptions.queryOptions()
                .scanConsistency(QueryScanConsistency.REQUEST_PLUS)
                .parameters(parameters);
        return cluster.query(query, options).rowsAsObject()
                .stream()
                .map(JsonObject::toMap)
                .toList();
    }
