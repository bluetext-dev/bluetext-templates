...
import { fetchTemplates, addService, type Template } from "../api";
...
export function AddServiceOverlay({
  ...
  const handleAdd = async (template: Template) => {
    ...
    try {
      const result = await addService(template.id, id);
      if (result.success) {
        onToast(result.message ?? `Added service "${id}"`, "success");
      } else {
        onToast(result.error ?? result.message ?? "Failed to add service", "error");
      }
    } catch {
    ...
    <div className="overlay-backdrop" onClick={onClose}>
      <div className="overlay-panel" onClick={(e) => e.stopPropagation()}>
        <div className="overlay-header">
          <span>Add Service from Template</span>
