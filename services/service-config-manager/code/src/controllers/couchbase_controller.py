...
class CouchbaseController:
    ...
    def __init__(self, environment: str, config: Config, service_name: str, config_dir: str):
        ...
        self.mgmt_port = self._get_env_var(f'{self.prefix}_MGMT_PORT', '18091' if self.tls else '8091')
    ...
    def _rename_node(self) -> None:
        """Rename the Couchbase node to use the service FQDN so other pods can reach it."""
        import base64
        protocol = "https" if self.tls else "http"
        port = self.mgmt_port
        rename_url = f"{protocol}://{self.host}:{port}/node/controller/rename"

        credentials = base64.b64encode(f'{self.username}:{self.password}'.encode()).decode('ascii')
        encoded_data = urllib.parse.urlencode({'hostname': self.host}).encode()
        request = urllib.request.Request(rename_url, data=encoded_data, method='POST')
        request.add_header('Authorization', f'Basic {credentials}')

        try:
            with urllib.request.urlopen(request, timeout=30) as response:
                self.logger.info(f"✅ Node renamed to {self.host}")
        except urllib.error.HTTPError as e:
            error_body = e.read().decode() if hasattr(e, 'read') else str(e)
            self.logger.warning(f"⚠️ Node rename returned HTTP {e.code}: {error_body}")
        except Exception as e:
            self.logger.warning(f"⚠️ Node rename failed: {e}")
